	...
	for line in file:
		...
		if subtype == 'INS' or subtype == "DUP":
			...
			if sum(result) == 3:
				nice_job_3 +=1
	file.close()
	print("[INFO]: (1,2,3) %d,%d,%d signal locuses Sniffles are good!"%(nice_job_1, nice_job_2, nice_job_3))
	Tp = 0
	Np = 0
	for key in DICT[0]:
		for i in xrange(len(DICT[0][key])):
