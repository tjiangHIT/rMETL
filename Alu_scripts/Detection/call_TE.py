...
def call_vcf(args):
	...
	for i in sort_list:
		# print("%s\t%s\t%s\t%s"%(i[0], breakpoint, insert_size, final_type))
		# print "\t".join(i)
		INFO = "SVTYPE=%s;SVLEN=%d;END=%d;SAMPLE=%s;STRAND=%s"%(i[3][1:4], int(i[2]), int(i[1])+int(i[2])-1, args.sample, '+/-')
		...
		file.write("%s\t%s\t%d\tN\t%s\t.\t.\t%s\tGT:DV:DR\t%s:%s\n"%(i[0], i[1], ID, i[3], INFO, GT, GL))
		ID += 1
